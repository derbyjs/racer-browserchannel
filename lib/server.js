...
module.exports = function(store, options) {
  ...
  var middleware = browserChannel(options, function(client) {
    ...
    var stream = createBrowserChannelStream(client, store.passthrough);
    store.shareClient.listen(stream);
  });
  return middleware;
};

function createBrowserChannelStream(client, passthrough) {
  ...
  return passthrough ? stream.pipe(passthrough) : stream;
}
